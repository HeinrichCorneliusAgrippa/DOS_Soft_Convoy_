...
# Convoy scenes run two bots in one Godot process, so each must read its OWN
# ports. Set role="lead"/"follow" per bot in the scene and the launcher passes
# role-prefixed args (--lead-camera-port=, --follow-camera-port=, ...). Left
# empty ("") for single-bot scenes, which keep reading the un-prefixed args.
...
func _enter_tree() -> void:
	var cam_key := "--camera-port="
	var whl_key := "--wheel-port="
	var pf_key  := "--port-file="
	if role != "":
		cam_key = "--%s-camera-port=" % role
		whl_key = "--%s-wheel-port="  % role
		pf_key  = "--%s-port-file="   % role
	for arg in OS.get_cmdline_user_args():
		if arg.begins_with(cam_key):
			CameraPort = int(arg.split("=")[1])
		elif arg.begins_with(whl_key):
			WheelPort = int(arg.split("=")[1])
		elif arg.begins_with(pf_key):
			port_file_path = arg.split("=")[1]
	print("[Robot:", role, "] Ports from CLI: camera=", CameraPort, " wheel_hint=", WheelPort, " port_file=", port_file_path)
